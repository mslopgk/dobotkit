...
class MotionMixin(_Base):
    ...
    def set_ptp_cmd(self, mode: int, x: float, y: float, z: float, r: float,
                    queued: bool = True, wait_finish: bool = False) -> int:
        """Send PTP (point-to-point) motion command; returns queued command index.

        When ``wait_finish`` is True, ``isWaitForFinish=true`` is sent so
        DobotLink blocks until the arm physically completes the move (matching
        DobotLab's own behaviour), and the call is given a longer timeout since
        a move can take several seconds.
        """
        call_timeout = 20.0 if wait_finish else None
        return self._queued_index(self._call(
            "SetPTPCmd", call_timeout=call_timeout,
            ptpMode=int(mode), x=float(x), y=float(y), z=float(z), r=float(r),
            isQueued=queued, isWaitForFinish=wait_finish))
    ...
    def set_home_cmd(self, queued: bool = True, wait_finish: bool = False) -> int:
        """Send home command; returns queued command index.

        When ``wait_finish`` is True, ``isWaitForFinish=true`` is sent so
        DobotLink blocks until homing physically completes; homing is slow
        (~15-20s), so a longer timeout is used.
        """
        call_timeout = 40.0 if wait_finish else None
        return self._queued_index(self._call(
            "SetHOMECmd", call_timeout=call_timeout,
            isQueued=queued, isWaitForFinish=wait_finish))
