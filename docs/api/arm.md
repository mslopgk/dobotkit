...
- **프로토콜 명령 ID는 재구성된 값입니다.** 명령 ID는 (a) `pydobot` 시드(독립 검증된 값), (b) `DobotDllType`의 ctypes 구조체 레이아웃, (c) 문서화된 카테고리별 번호 규칙으로 재구성했습니다. **pydobot 시드로 확인되지 않은 ID 값은 코드에서 `# unverified` 주석으로 태깅**되어 있으며, 펌웨어/공식 프로토콜 문서 또는 실제 하드웨어로 확인이 필요합니다.
...
> 요약: **구조체 바이트 레이아웃은 검증됨**, **명령 ID 중 일부는 미검증(`# unverified`)** — 실제 하드웨어로 확인하세요.
